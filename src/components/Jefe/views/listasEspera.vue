...
import TablaEspera from "../components/tablaEspera.vue";
import { ref, onMounted } from "vue";

export default {
  components: { Lateral, Encabezado, TablaEspera },
  setup() {
    const JefeInfo = ref();
    const jefeEs = async () => {
        ...
        for(let i = 0; i < estadoProcesos.length; i++){
          if(estadoProcesos[i].idProceso == 2){
            if(estadoProcesos[i].estado == true){
              ...
              window.alert("El proceso de matrículas no está activo, no puede acceder a esta página");
              window.history.back();
            }else{
              ...
              if(comprobacion == true){
                console.log("El proceso de matrículas está activo");
              }else{
                window.alert("El proceso de matrículas no está activo");
                window.history.back();
              }
            }
