  ...
  setup() {
    ...
    const comprobacionEstado = async ( ) =>{
      try {
        ...
        if(estadoProcesos[3].estado == true  || estadoProcesos[1].estado == true ){
          //obtener el inicio del proceso de planificacion
          let fechaInicio = new Date(estadoProcesos[3].fechaInicioDelProceso)
          fechaInicio.setDate(fechaInicio.getDate() + 1)

          //obtener el fin del proceso de matricula
          let fechaFin = new Date(estadoProcesos[1].fechaFinDelProceso)
          fechaFin.setDate(fechaFin.getDate() + 1)
          
          let intervaloDeFecha = [];

          //obtener el intervalo de fechas entre el inicio y el fin de los procesos
          ...
          console.log("intervalos de fecha", intervaloDeFecha)
          ...
          if(comprobacion == true){
            console.log("El proceso de planificación esta activo");
          }else{
            window.alert("El proceso de planificación no esta activo");
            window.history.back();
          } 
        }else{
          window.alert("El proceso de planificación no esta activo");
        ...
        //   if(estadoProcesos[i].idProceso == 4 && estadoProcesos[i].idEstadoProceso == 2){
        //     if(estadoProcesos[i].estado == true){
        //       if(estadoProcesos[i].fechaInicioDelProceso == null || estadoProcesos[i].fechaFinDelProceso == null){
        //       window.alert("El proceso de planificación no está activo");
        //       window.history.back();
        //     }else{
