          ...
          <v-col class="left-side">
            <v-row class="row-logo mr-auto ml-auto">
              ...
              <v-col cols="12" lg="12" md="12">
                <h1 class="bien text-center">
                  ¡Bienvenidos Pumas {{ puesto }}!
                </h1>
              </v-col>
              <v-col cols="12" lg="12" md="12">
                <router-link to="/"
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      const direcciones = {
        "/login-docentes": "/docentes",
        "/login-coordinador": "/principalCoordinador",
        "/login-administrador": "/administrador",
        "/login-jefeDep": "/principalJefe",
      };
    ...
    login: async (numeroEmp, claveEmp, router, nombre, dir) => {
      ...
      try {
        ...
        if (
          ...
          if (dir === "/principalCoordinador") {
            const infoUsuario = JSON.stringify(data.coordinador);
            const infoCoordinador = JSON.stringify(data.docente);
            console.log(infoUsuario);
            localStorage.setItem("Coordinador", infoUsuario);
            localStorage.setItem("CoordinadorInfo", infoCoordinador);
          }
          ...
          if (dir === "/principalJefe") {
            const infoUsuario = JSON.stringify(data.jefe);
            const infoJefe = JSON.stringify(data.docente);
            console.log(infoUsuario);
            localStorage.setItem("JefeDep", infoUsuario);
            localStorage.setItem("JefeInfo", infoJefe);
          }
          router.push(dir);
        }
      } catch (error) {
