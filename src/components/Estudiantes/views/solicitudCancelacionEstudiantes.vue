...
import FormCancelacionExcepcional from '../components/formCancelacion.vue'
import { ref,onMounted } from 'vue'

export default {
components:{Lateral,Encabezado,FormCancelacionExcepcional},
setup(){
          ...
          if(estadoProcesos[i].idProceso == 1){
            if(estadoProcesos[i].estado == true){
              if(estadoProcesos[i].fechaInicioDelProceso == null || estadoProcesos[i].fechaFinDelProceso == null){
                window.alert("El periodo de cancelaciones excepcionales no está activo");
                window.history.back();
              }else{
              ...
              if(comprobacion == true){
                console.log("El periodo de cancelaciones excepcionales está activo");
              }else{
                window.alert("El periodo de cancelaciones excepcionales no está activo");
                window.history.back();
              }
